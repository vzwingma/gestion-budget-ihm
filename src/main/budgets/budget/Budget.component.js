...
export default class Budget extends Component {
    ...
    shouldComponentUpdate(nextProps, nextStates, nextContext) {
        ...
        } else if (this.state.currentOperation !== nextStates.currentOperation) {
            console.log("[TRIGGER] Context operation=" + (nextStates.currentOperation !== null ? nextStates.currentOperation.id : "Nouvelle opération"))
            componentUpdate = true;
        } else if (this.state.filterOperations !== nextStates.filterOperations) {
            componentUpdate = true;
        }
                    ...
                    <Grid2 md={3} paddingTop={"6px"}>
                        <Paper component="form"
                            ...
                            <CancelRounded sx={{color: "#D0D0D0", cursor: "pointer"}}
                                           onClick={() => this.setState({filterOperations: null})}/>
                ...
                <Grid2 container sx={{overflow: "hidden"}}>
                    <Grid2 md={4} direction={"column"} sx={{overflow: "hidden"}} maxHeight>
                        { /** Liste des opérations **/
                            (this.state.currentBudget != null ?
                                    <OperationsListe
                                        operationGroupedByDate={this.state.operationsGroupedByDateOperation}
